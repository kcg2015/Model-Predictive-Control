    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          vector<double> ptsx = j[1]["ptsx"];//waypoint x positions
          vector<double> ptsy = j[1]["ptsy"];//waypoint y positions
          double px = j[1]["x"];
          double py = j[1]["y"];
          double psi = j[1]["psi"];
          ...
          // Solve the MPC optimization with state and coeffs as inputs
          auto sol_vec = mpc.Solve(state, coeffs);
          
    
          double steer_value=sol_vec[0];
          double throttle_value = sol_vec[1];
          double N = sol_vec[2]; // no. of steps in MPC.cpp
          ...
          // Add (x,y) points , points are in reference to the vehicle's coordinate system
          // the points in the simulator are connected by a Green line
 
          for (int i = 0; i < N-1; i++) {
            mpc_x_vals.push_back(sol_vec[3 + i]);
            mpc_y_vals.push_back(sol_vec[3 + i + N]);
          }
